...
import { clearChunkPool, getChunkPoolSize } from "./stream.js";
...
app.use((req, res, next) => {
  ...
  if (status.blocked) {
    ...
    return res.status(429).json({
      error: `Access Denied: Temporarily blocked for ${remainingMinutes} minutes.`,
    });
  }
  next();
});
  ...
  if (req.path.startsWith("/v1/") || req.path.startsWith("/cli/v1/")) {
    ...
    if (apiKey) {
      ...
      if (providedKey !== apiKey) {
        ...
        logger.warn(
          `API Key 验证失败: ${req.method} ${req.path} (提供的Key: ${providedKey ? providedKey.substring(0, 10) + "..." : "无"})`,
        );
