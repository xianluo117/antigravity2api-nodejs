  ...
  for (const [key, value] of Object.entries(obj)) {
    ...
    if (key === "type") {
      ...
      if (typeof value === "string") {
        // 字符串类型：转换为大写
        cleaned[key] =
          TYPE_UPPERCASE_MAP[value.toLowerCase()] || value.toUpperCase();
        ...
        if (nonNullType && typeof nonNullType === "string") {
          cleaned[key] =
            TYPE_UPPERCASE_MAP[nonNullType.toLowerCase()] ||
            nonNullType.toUpperCase();
        } else {
      ...
      cleaned[key] =
        value && typeof value === "object" ? cleanParameters(value) : value;
    }
  }
  return cleaned;
}
...
export function modelMapping(modelName) {
  modelName = stripPublicModelPrefix(modelName);
  ...
  const pattern1 = modelName.match(
  ...
  const pattern2 = modelName.match(
    /^claude-\d+-\d+-(sonnet|opus|haiku)-\d{8}$/,
  );
  if (pattern2) {
  ...
  if (modelName === "claude-opus-4-5-thinking")
    return "claude-opus-4-6-thinking";
  if (modelName === "claude-opus-4-6") return "claude-opus-4-6";
...
export function generateGenerationConfig(
  ...
  if (
    normalizedParams.thinking_budget === undefined &&
    parameters.reasoning_effort !== undefined
  ) {
    const defaultThinkingBudget = config.defaults.thinking_budget ?? 1024;
    normalizedParams.thinking_budget =
      REASONING_EFFORT_MAP[parameters.reasoning_effort] ??
